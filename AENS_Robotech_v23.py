...
def dronpoints_detection(img):
    ...
    upper_green = np.array([70, 240, 220])
    mask_green = cv2.inRange(hsv, lower_green, upper_green)
    res_green = cv2.bitwise_and(hsv, hsv, mask=mask_green)
    res_green = cv2.cvtColor(res_green, cv2.COLOR_BGR2GRAY)
...
def main():
    ...
    for i in range(9):
        nextMark(0.45*i, 4.95, zc=INVENTARIZATION_ALTITUDE, speed=INVENTARIZATION_SPEED, tolerance=0.1)
        logging('INV on')
