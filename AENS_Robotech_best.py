...
def nextMark(xc, yc, zc=ALTITUDE):
    navigate_wait(x=float(xc), y=float(yc), z=float(zc), yaw=0.0, speed=SPEED, frame_id='aruco_map',  auto_arm=True, tolerance=0.1)
    rospy.sleep(0.5)
...
def image_callback(data):    
    ...
    if INVENTARIZATION_ENABLE:
        ...
        min_height, max_height = height//2 - height//(INVENTARIZATION_CROP_IMAGE*2),\
                                 height//2 + height//(INVENTARIZATION_CROP_IMAGE*2)
...
def main():
    ...
    for i in range(9):
        nextMark(0.45*i, 4.95, INVENTARIZATION_ALTITUDE)
        print 'INV on'
