...
def scan_cargo(img):
    ...
    ret, res_green = cv2.threshold(res_green, 70, 255, 0)
    ...
    lower_yellow = np.array([25, 50, 90])
    ...
    lower_blue = np.array([105, 75, 100])
    ...
    ret, res_blue = cv2.threshold(res_blue, 90, 255, 0)#110
    ...
    for point in points:
        ...
        if DETECTED_DRONPOINT != -1:  # Нашли дронпоинт
            ...
            if DRONPOINTS_LANDING and cargo_amount[COLOR[DETECTED_DRONPOINT]] > 0:
                ...
                color_blinking(COLOR[DETECTED_DRONPOINT])
                take_off()
                time.sleep(1)
